# multi-drive-import Specification

## Purpose

Extends `scan` and `import` so a `source: auto` profile detects and sequentially processes every mounted volume matching the device implementation's `detect()`, not just the first one found — each drive gets its own named header, its own scan/plan/execute cycle, and its own confirmation and failure handling, all within a single command invocation.
